...
fn can_send_datagram_to_service() -> Result<()> {
    ...
    let (mut session, mut second_session) = create_two_sessions(&test_name, udp_socket.local_addr()?.port(), second_udp_socket.local_addr()?.port())?;
...
fn can_deserialize_datagram_message() -> Result<()> {
    ...
    let example_received_datagram_bytes = [
        0x44, 0x41, 0x54, 0x41, 0x47, 0x52, 0x41, 0x4d, 0x20, 0x52, 0x45, 0x43, 0x45, 0x49, 0x56,
        0x45, 0x44, 0x20, 0x44, 0x45, 0x53, 0x54, 0x49, 0x4e, 0x41, 0x54, 0x49, 0x4f, 0x4e, 0x3d,
        0x64, 0x65, 0x73, 0x74, 0x69, 0x6e, 0x61, 0x74, 0x69, 0x6f, 0x6e, 0x20, 0x53, 0x49, 0x5a,
        0x45, 0x3d, 0x6e, 0x75, 0x6d, 0x42, 0x79, 0x74, 0x65, 0x73, 0x0a, 0x05, 0x10
    ];
